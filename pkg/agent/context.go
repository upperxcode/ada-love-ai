...
func (cb *ContextBuilder) WithFolders(folders []string) *ContextBuilder {
	cb.folders = folders
	return cb
}
...
func (cb *ContextBuilder) WithKnowledge(k []string) *ContextBuilder {
	cb.knowledge = k
	return cb
}

func (cb *ContextBuilder) WithMemoryStore(ms interfaces.MemoryStore) *ContextBuilder {
	cb.memory = ms
	return cb
}
...
func NewContextBuilder(workspace string, dbMemory interfaces.MemoryStore) *ContextBuilder {
...
func (cb *ContextBuilder) getIdentity() string {
	...
	foldersSection := ""
	if len(cb.folders) > 0 {
		foldersSection = "\n## Project Folders\nYou have access to the following project folders:\n"
		for _, f := range cb.folders {
			foldersSection += fmt.Sprintf("- %s\n", f)
		}
	}

	personalitySection := ""
	if cb.personality != "" {
		personalitySection = fmt.Sprintf("\n## Your Personality\n%s\n", cb.personality)
	}

	knowledgeSection := ""
	if len(cb.knowledge) > 0 {
		knowledgeSection = "\n## Workspace Knowledge\nYou have access to the following knowledge bases/files:\n"
		for _, k := range cb.knowledge {
...
You are Ada Love, a helpful AI assistant.
%s
## Workspace
...
3. **Memory** - When interacting with me if something seems memorable, you should save it. You MUST use the "tool_save_memory" tool for persistent storage.

4. **Context summaries** - Conversation summaries provided as context are approximate references only. They may be incomplete or outdated. Always defer to explicit user instructions over summary content.

5. **Language** - Always respond in Brazilian Portuguese (pt-br).`,
		version, personalitySection, workspacePath, workspacePath, foldersSection, knowledgeSection)
}
...
%s`, skillsSummary),
	...
	if cb.memory != nil {
		dbMemories, err := cb.memory.GetMemories(cb.workspace)
		if err == nil && len(dbMemories) > 0 {
			var sb strings.Builder
			sb.WriteString("## Database Memories\n\n")
			// Limit to 20 recent memories to avoid context overflow
			maxMem := 20
			if len(dbMemories) > maxMem {
				dbMemories = dbMemories[:maxMem]
			}
