...
func TestLegacyCompact_PostTurn_ExceedsMessageThreshold(t *testing.T) {
	...
	al := NewAgentLoop(cfg, msgBus, &simpleMockProvider{response: "summary"}, nil)
...
func TestIngestCalledDuringTurn(t *testing.T) {
	...
	msgBus := bus.NewMessageBus()
	al := NewAgentLoop(cfg, msgBus, &simpleMockProvider{response: "done"}, nil)
	defaultAgent := al.registry.GetDefaultAgent()
	if defaultAgent == nil {
		t.Fatal("expected default agent")
	}
...
func newCMTestAgentLoop(cfg *config.Config) *AgentLoop {
	msgBus := bus.NewMessageBus()
	return NewAgentLoop(cfg, msgBus, &simpleMockProvider{response: "test"}, nil)
}
