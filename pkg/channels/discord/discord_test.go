...
func TestAppendContent(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := appendContent(tt.content, tt.suffix)
			if got != tt.expected {
				t.Errorf("appendContent(%q, %q) = %q, want %q", tt.content, tt.suffix, got, tt.expected)
			}
		})
	}
}
...
func TestOutboundMessageIsToolFeedback(t *testing.T) {
	tests := []struct {
		...
		{
			name: "not tool feedback - empty raw",
			msg: bus.OutboundMessage{
				Content: "hello",
				Context: bus.InboundContext{
					Raw: nil,
		...
		{
			name: "not tool feedback - other kind",
			msg: bus.OutboundMessage{
				Content: "hello",
				Context: bus.InboundContext{
		...
		{
			name: "tool feedback case insensitive",
			msg: bus.OutboundMessage{
				Content: "using tool X",
				Context: bus.InboundContext{
					Raw: map[string]string{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got != tt.isTFB {
				t.Errorf("outboundMessageIsToolFeedback() = %v, want %v", got, tt.isTFB)
			}
		})
	}
}

func TestDiscordMaxMessageLength(t *testing.T) {
	ch := &DiscordChannel{
		BaseChannel: channels.NewBaseChannel("discord", &config.DiscordSettings{}, bus.NewMessageBus(), nil,
			channels.WithMaxMessageLength(2000)),
	}
	maxLen := ch.MaxMessageLength()
	if maxLen != 2000 {
		t.Errorf("MaxMessageLength() = %d, want 2000", maxLen)
	}
}
