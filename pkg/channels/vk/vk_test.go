...
func TestVKIsMentioned(t *testing.T) {
	msgBus := bus.NewMessageBus()
	bc := makeVKTestBaseChannel(config.VKSettings{
		Token:   *config.NewSecureString("test_token"),
		GroupID: 123456789,
	...
	result := ch.isMentioned(msg)
	if result != false {
		t.Errorf("isMentioned() = %v, want false (not implemented)", result)
	}
}

func TestVKStripBotMention(t *testing.T) {
	msgBus := bus.NewMessageBus()
	bc := makeVKTestBaseChannel(config.VKSettings{
		Token:   *config.NewSecureString("test_token"),
		GroupID: 123456789,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result != tt.expected {
				t.Errorf("stripBotMention(%q) = %q, want %q", tt.input, result, tt.expected)
			}
		})
	}
}

func TestVKNewVKChannelErrors(t *testing.T) {
	msgBus := bus.NewMessageBus()
...
func TestVKGetVKCfg(t *testing.T) {
	msgBus := bus.NewMessageBus()
	bc := makeVKTestBaseChannel(config.VKSettings{
		Token:   *config.NewSecureString("test_token"),
		GroupID: 123456789,
	...
	cfg := ch.getVKCfg()
	if cfg == nil {
		t.Fatal("getVKCfg() should not return nil")
	}
	if cfg.GroupID != 123456789 {
		t.Errorf("getVKCfg().GroupID = %d, want 123456789", cfg.GroupID)
	}
}
