...
func TestWeixinMaxMessageLength(t *testing.T) {
	ch := &WeixinChannel{
		BaseChannel: basechannels.NewBaseChannel("weixin", config.WeixinSettings{}, &bus.MessageBus{}, nil,
			basechannels.WithMaxMessageLength(4000)),
	}
...
func TestWeixinSplitMessage(t *testing.T) {
	tests := []struct {
		name    string
		content string
		maxLen  int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := basechannels.SplitMessage(tt.content, tt.maxLen)
			if len(got) != tt.want {
...
func TestWeixinVoiceCapabilities(t *testing.T) {
	ch := &WeixinChannel{
		BaseChannel: basechannels.NewBaseChannel("weixin", config.WeixinSettings{}, &bus.MessageBus{}, nil),
	}
...
func TestNewWeixinChannel(t *testing.T) {
	t.Run("valid config", func(t *testing.T) {
		...
		ch, err := NewWeixinChannel(bc, cfg, &bus.MessageBus{})
		if err != nil {
